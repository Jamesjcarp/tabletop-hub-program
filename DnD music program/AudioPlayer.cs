...
namespace DnD_music_program
{
    internal class AudioPlayer
    {
        ...
        public AudioPlayer()
        {
            ...
            // subscribe to the UI events so that it is updated when the UI is.
            UI.PlayTrack += PrepMusicWorker;
            UI.Stop += StopTrack;
            UI.VolumeMusicChanged += ChangeMusicVolume;
            UI.VolumeSoundEffectChanged += ChangeSoundEffectVolume;
            UI.PlaySound += PrepSoundEffectWorker;
        ...
        private static void ChangeMusicVolume(int volume)
        {
            musicVolume = volume;
            music.Volume = musicVolume;
        }

        private static void ChangeSoundEffectVolume(int volume)
        {
        ...
        private static void PrepMusicWorker(string title)
        {
        ...
        private static void PlayTrack(string title, CancellationToken cancelTok)
        {
            ...
            // songs with no intro are represented by the intor path being "NULL"
            if (songPaths[1] == "NULL")
            {
        ...
        private static void PrepSoundEffectWorker(string title)
        {
